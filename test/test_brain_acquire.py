...
import requests
import json
from common import  brain_ip, brain_port

class TestBrainAcquire(unittest.TestCase):
    def setUp(self) -> None:
        self.proxies={"http": None, "https": None}
        url = "http://{}:{}/sensitize_list".format(brain_ip, brain_port)
        re = requests.get(url, proxies=self.proxies)
        if re.status_code != 200:
            print("ERROR: Can't reach KeenTune-Brain.")
            exit()
            
        url = "http://{}:{}/{}".format(brain_ip, brain_port, "init")
        data = {
            "algorithm":"tpe",
            "iteration":500,
            "name":"test",
    ...
    def tearDown(self) -> None:
        url = "http://{}:{}/{}".format(brain_ip, brain_port, "end")
        result = requests.get(url, proxies=self.proxies)
        self.assertEqual(result.status_code, 200)
        self.assertEqual(result.text, '{"suc": true, "msg": ""}')

    def test_brain_server_FUN_acquire(self):
        url = "http://{}:{}/{}".format(brain_ip, brain_port, "acquire")
        result = requests.get(url, proxies=self.proxies)
        self.assertEqual(result.status_code, 200)
